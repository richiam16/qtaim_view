...
C     USING THE FACTORS COMPUTED BY WDGECO OR DGEFA.
C
C     ON ENTRY
C
C        A       DOUBLE PRECISION(LDA, N)
...
C
      DO 10 J=1,Iplac*NMO
      CO(IPSI+J)=ZERO -------> ¿valor zero?
10    CONTINUE
C
      DO 20 I=1,NPRIMS
      K=IC(ICENT+I)---------> tipo+nprimitivas
      ENT=CO(IE+I)-----------> exponentes
      X=CO(IXX+K)------------>coordenadas entre centro y punto
      Y=CO(IYY+K)
      Z=CO(IZZ+K)
      EXPON=DEXP(-ENT*CO(IR2+K))------->exponencial de exponentes y distancia(respecto a un punto)
C
      Call PrimeF(Ent,Expon,X,Y,Z)----->Entrada:exponentes,exponencial,coordenadas, derivadas de funciones gaussianas sin coeficientes
C
      IWhich=IC(ITYPE+I)--------> tipo de orbital gaussiano
C
      Call PrimeA(IWhich,Ifill,X,Y,Z)------Derivadas de funciónes ¿A?
C
      Call PrimeG--------Derivadas de A junto con las exponenciales
C
      Call PrimeA(IWhich,IEmpty,X,Y,Z)
C
      Check=Dmax1(dabs(G0),dabs(Gx),dabs(Gy),dabs(Gz))------------>máximo de primeras derivadas
      If(check*CO(Icofmx+I).gt.cutoff)Then-------si check*4 derivada de G(z)==cutoff(input)
      DO 30 J=1,NMO
      CIJ=CO(IMO+NPRIMS*(J-1)+I)----------->toma los coeficientes del orbital molecular
      CO(IPSI+J)=CO(IPSI+J)+CIJ*G0--------->crea puntos de psi
      CO(IGX+J)=CO(IGX+J)+CIJ*GX----------->deriva psi (x)
      CO(IGY+J)=CO(IGY+J)+CIJ*GY
30    CO(IGZ+J)=CO(IGZ+J)+CIJ*GZ
      Endif
      If(Iwhat.gt.0)Then-----------------segunda derivada
...
40    CO(IGZZ+J)=CO(IGZZ+J)+CIJ*GZZ
      Endif
      Endif
      If(Iwhat.gt.1)Then---------------tercera derivada
...
50    CO(IGZZZ+J)=CO(IGZZZ+J)+CIJ*GZZZ
      Endif
      Endif
      If(Iwhat.gt.2)Then---------------cuarta derivada
...
99    DO 100 I = 1,MaxSch
        IS = IS + 1	
        CALL DGECO(H,3,3,IW,ER,R1) #(H=matriz,3=dimension,3=orden,inidice-pivote,prec,prec)--->Factoriza
        CALL DGEDI(H,3,3,IW,,R1,1) #(H=matriz de DGECO,dimension,orden,pivote-,determonante,vector trabjo,determinante-inversa)----->Inversa
        DO 110 J = 1,3
          R1(J) = Zero
          DO 110 K = 1,3
            R1(J) = R1(J) + H(J,K)*W(K)
...
C
      ...
      FZ=toalpe*Z
      If(Iwhat.gt.0)Then
      FXX=toalp*X*FX+toalpe------>-two*ent*expon*x------->x*x*-two*ent*-two*ent*expon+(-two)*ent*expon---->x*x*toalp*toalp*expon+toalp*expon--->toalpe*toalp*x*x+toalpe--->fx*x*toalp+toalpe
      FXY=toalp*Y*FX
      FXZ=Toalp*Z*FX
...
C
      ...
      IGYZZZ=IGYYZZ+NMO
      IGZZZZ=IGYZZZ+NMO
      ICOFMX=IGZZZZ+NMO-------->4 derivada de g respecto a z
C
      DO 100 I = 1,NCENT

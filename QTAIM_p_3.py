...
class Mol:
        ...
        def rdwfn(self): 
                ...
                for i in range(2+int(self.num_nucl),len(lines)): #del final de las lineas de nucleos, hasta el fin del documento
                        clasi=lines[i].split()
                        if clasi[0]=="CENTRE":
        ...
        def graph(self,en=0,xyz,min,max,pun,lim_rho,cont=0,min_c=30):
                ...
                if xyz=="xy":
                        ...
                        if float(cont)==1:
                                ...
                                cs=ax.contour(val_x,val_y,self.grap[0],int(min_c),cmap=cm.coolwarm)
                        else:
                                # ax=fig.gca(projection="3d")
                                ax = fig.add_subplot(projection='3d')
                                surf=ax.plot_surface(val_x,val_y,self.grap[0],cmap=cm.coolwarm)
                if xyz=="xz":
